...
export default function MakeResumePage() {
  ...
  const downloadDocument = (format: 'pdf') => {
    ...
    if (!resumeDocument.content && !Object.values(resumeDocument.sections).some(section => section)) {
      alert('No content to download');
      return;
    }

    if (format === 'pdf') {
      console.log(resumeDocument.content);
      const documentContent = resumeDocument.content || 
        `${resumeDocument.sections.header || ''}<br><br>${resumeDocument.sections.summary || ''}<br><br>${resumeDocument.sections.experience || ''}<br><br>${resumeDocument.sections.education || ''}<br><br>${resumeDocument.sections.skills || ''}`;
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/resume/v2', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        const assistantMessage: Message = {
          role: 'assistant',
          content: data.chat_response || 'I received your message but didn\'t have a response. Let me help you with your resume.',
          timestamp: new Date().toISOString()
        };
        ...
        if (data.document_updates) {
          setResumeDocument(data.document_updates);
        }
      } else {
    ...
    <div className="p-1 h-full w-full bg-sidebar">
      <div className="flex flex-col h-full w-full bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="flex w-full border-b border-gray-200">
          ...
          <p className="text-sm"><span className="font-semibold">Make Resume</span> / New Resume</p>
          {/* Download menu */}
          ...
          <div className="flex w-[25%] justify-between items-center border-l border-gray-200 h-12">
            <div className="text-sm font-medium flex-1 h-full items-center flex justify-start px-2">
              <span>Chat with resume AI agent</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 flex justify-center items-start p-8 pt-4">
            <div className="bg-white rounded-lg shadow-lg max-w-4xl w-full min-h-[600px]">
              {resumeDocument.content ? (
                <DocViewer 
                  htmlContent={resumeDocument.content}
                  onChangeHtml={(next) => setResumeDocument(prev => ({
                    ...prev,
                    content: next
                  }))}
                />
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-sm font-medium text-gray-600">
                    Start chatting with AI to create your ATS-friendly resume
                  </p>
                </div>
              )}
            </div>
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm text-center font-medium text-gray-600">
                  Start a conversation with AI to build your ATS-friendly resume
                </p>
              </div>
            ) : (
            ...
            <div className="bg-white rounded-lg shadow-sm border border-[#E7E6E4] p-1.5">
              <div className="flex items-end gap-2">
                  <textarea
                    ...
                    placeholder="Chat with our resume AI agent..."
                    rows={inputRows}
